...
class SmartCurtain {
  ...
  public:
    ...
    void begin()
    {
      curtainServo.attach(pin);  // 将舵机连接到指定的引脚
      curtainServo.write(0);     // 初始化时设置窗帘为关闭状态
    }
    ...
    void openCurtain() {
      for (int pos = 0; pos <= 180; pos++) {
        curtainServo.write(pos);  // 设置舵机的位置
...
Fan myFan(MOTOR_PIN);
SmartCurtain myCurtian(CURTAIN_PIN);
int pos = 0;
